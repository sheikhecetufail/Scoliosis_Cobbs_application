...
class ImageProcessingApp:
    def __init__(self, root):
        ...
        # self.vertical_scrollbar = tk.Scrollbar(root, orient="vertical", command=self.vertical_scroll)
        # self.vertical_scrollbar.pack(side="right", fill="y")
        
        # self.vertical_scrollbar = tk.Scrollbar(root, orient="vertical", command=self.canvas.yview)
        # self.vertical_scrollbar.pack(side="right", fill="y")

        # # Connect the vertical scrollbar to the canvas
        ...
        #self.binary_image_label.config(yscrollcommand=self.vertical_scrollbar.set)
    ...
    def capture_intersection(self, event):
        ...
        if len(self.intersection) == 1:
            ...
            # point_i = self.intersection[0]
            # point_i = tuple(v*2 for v in point_i)
            # image_with_line = self.processed_image.copy()
            # cv2.circle(image_with_line, point_i,2, (0, 255, 0), 10)  # Green 
    ...
    def angle(self):
        ...
        p1 = self.coordinates[1]
        p1 = tuple(v*2 for v in p1)
        p2 = self.coordinates[3]
        p2 = tuple(v*2 for v in p2)
        p3 = self.intersection[0]
        p3 = tuple(v*2 for v in p3)

        p1_2 = self.coordinates[0]
        p1_2 = tuple(v*2 for v in p1_2)
        p2_2 = self.coordinates[2]
        p2_2 = tuple(v*2 for v in p2_2)
